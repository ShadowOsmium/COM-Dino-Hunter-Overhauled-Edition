...
public class iDataCenter    
{
    ...
    public void SetHunterLevel(int targetLevel)
    {
        if (targetLevel < 1) targetLevel = 1;

        int currentLevel = m_nHunterLvl.Get();

        // If already at or above target level, do nothing
        if (currentLevel >= targetLevel)
            return;

        // Increase level by 25% towards the target level
        int levelIncrease = (int)Math.Ceiling((targetLevel - currentLevel) * 0.25);
        int newLevel = currentLevel + levelIncrease;

        // Ensure new level does not exceed target level
        if (newLevel > targetLevel)
            newLevel = targetLevel;

        Console.WriteLine(string.Format("Setting Hunter Level: {0} → {1}", currentLevel, newLevel));
        m_nHunterLvl.Set(newLevel);
    }

    public void LoadHunterLevelFromSave(string value)
    {
        int parsedValue;
        if (int.TryParse(value, out parsedValue))
        {
            SetHunterLevel(parsedValue);
        }
    }
	...
	public void Clear()
	{
		...
		m_sSignature = "Modded by Nick (Discord v_o_r_t_e_x). Enjoy the rebalance!";
		m_ltFriends.Clear();
		m_ltTitle.Clear();
		m_ltTitle.Add(1);
		m_nTitle.Set(1);
